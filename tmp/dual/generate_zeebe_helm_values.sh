...
generate_initial_contact() {
    ...
    local zeebe_ingress_hostname_0=$1
    local zeebe_ingress_hostname_1=$2
    local release=$3
    local count=$4
    local port_number=443
    local result=""
    for ((i=0; i<count/2; i++)); do
        result+="${release}-zeebe-${i}.${zeebe_ingress_hostname_0}:${port_number},"
        result+="${release}-zeebe-${i}.${zeebe_ingress_hostname_1}:${port_number},"
    done
...
es_ingress_hostname_0=${CLUSTER_0_ES_INGRESS_DOMAIN:-""}
es_ingress_hostname_1=${CLUSTER_1_ES_INGRESS_DOMAIN:-""}

zeebe_ingress_hostname_0=${CLUSTER_0_ZEEBE_FORWARDER_DOMAIN:-""}
zeebe_ingress_hostname_1=${CLUSTER_1_ZEEBE_FORWARDER_DOMAIN:-""}
...
initial_contact=$(generate_initial_contact "$zeebe_ingress_hostname_0" "$zeebe_ingress_hostname_1" "$helm_release_name" "$cluster_size")
elastic0=$(generate_exporter_elasticsearch_url "$es_ingress_hostname_0")
elastic1=$(generate_exporter_elasticsearch_url "$es_ingress_hostname_1")

# TODO: later as an improvement, don't send traffic to ingress for the same cluster

if [[ "$mode" == "failover" ]]; then
    if [[ "$lost_region" == 0 ]]; then
        elastic0=$(generate_exporter_elasticsearch_url "$es_ingress_hostname_1")
        elastic1=$(generate_exporter_elasticsearch_url "$es_ingress_hostname_1")
    else
        elastic0=$(generate_exporter_elasticsearch_url "$es_ingress_hostname_0")
        elastic1=$(generate_exporter_elasticsearch_url "$es_ingress_hostname_0")
    fi
fi
